...
public class CodiceFiscaleChecker {
    ...
    /**
     * Metodo principale che controlla un codice fiscale qualsiasi per diversi
    ...
    public static boolean checkCodiceFiscale(String codiceFiscale) {
        if (!checkLunghezza(codiceFiscale))
            return false;
        else if (!checkNome(codiceFiscale))
            return false;
        else if (!checkCognome(codiceFiscale))
            return false;
        else if (!checkGiorno(codiceFiscale))
            return false;
        else if (!checkMese(codiceFiscale))
            return false;
        else if (!checkCodiceControllo(codiceFiscale))
            return false;
        else if (!checkPosizioneLettereNumeri(codiceFiscale))
            return false;
    ...
    public static boolean checkNomeCognome(String codice) {
        ...
        for (int i = 0; i < codice.length(); i++) {
            ...
            if (!isConsonante) {
                // Il boolean controlla che l'ultimo carattere sia una vocale, nel caso viene
                // controllato che il carattere
                // corrente sia anch'esso una vocale. Nel caso di consonante, il codice non è
                // valido.
    ...
    /**
     * Controlla se un carattere è una vocale o il carattere X, placeholder finale
     * per nomi di lunghezza inferiori a 3
     * 
     * @param carattere il carattere da controllare
     * @return true se il carattere è una consonante diversa da X, false se è una
     *         vocale o X
     */
    public static boolean isConsonanteAndNotX(char carattere) {
        // La X viene trattata come caso speciale, dato che dopo una vocale ci può
        // essere una X in nome e cognome
    ...
    /**
     * Controlla la presenza del carattere corretto corrispondente al mese, e se i
     * giorni corrispondono al mese presente
     * 
     * @param codiceFiscale il codice fiscale da controllare
    ...
    public static boolean checkMese(String codiceFiscale) {
        ...
        if (presente) {
            ...
            // A seconda del mese selezionato durante la verifica si controlla quanti giorni
            // ha
    ...
    /**
     * Controlla la correttezza dell'ultimo carattere di un codice fiscale
     * ricalcolandolo con l'algoritmo già impostato
     * 
     * @param codiceFiscale il codice fiscale da controllare
